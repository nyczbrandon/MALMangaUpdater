...
//Update user's myanimelist using the current manga the reader is currently on
function batoto_mal_updater(mal_basicauth, mal_username, id, page)
{
    return $.ajax({
    ...
    .done(function(data) {
        ...
        manga_list.then(function(data) {
            ...
            $(data).find('manga').each(function() {
                if (!manga_found && $(this).find('series_title').text() === manga_name) {
            ...
            if (!manga_found) {
                ...
                manga_search.then(function(data) {
                    ...
                    $(data).find('entry').each(function() {
                        if (!manga_found && $(this).find('title').text() === manga_name) {
                    ...
                    manga_add.then(function(data) {
                        ...
                        console.log("Manga not on your list");
                        return;
                    });
                }, function(data) {
...
function batoto_reader(mal_username, mal_basicauth) {
    ...
    if (split.length === 2)
        page = split[1];
    batoto_mal_updater(mal_basicauth, mal_username, id, page);
...
$(document).ready(function() {
    chrome.storage.local.get(['mal_username', 'mal_basicauth'], function(obj) {
        ...
        if (obj.mal_username && obj.mal_basicauth) {
            ...
            case 'bato.to':
                batoto_reader(obj.mal_username, obj.mal_basicauth);
                break;
            default:
